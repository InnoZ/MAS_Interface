class MatsimStarter
  ...
  JAVA_PATH = Rails.root.join('lib', 'matsim', 'java', 'innoz-toolbox-0.1-SNAPSHOT.jar')
  ...
  def run
    sleep(20)
    Kernel.system("java -mx4g -cp #{JAVA_PATH} com.innoz.toolbox.run.Main #{district_id} #{year} #{folder} #{rails_env}  >/dev/null 2>&1")
  end
